...
private val ru10 = object : IStr10 {
    ...
    override val guidePanicButtonDesc = "Третий, отдельный способ экстренного удаления (помимо пароля под принуждением и кнопок громкости) — постоянное уведомление с кнопкой «Удалить всё». Работает и на заблокированном экране, но не мгновенно: нужно развернуть уведомление, чтобы увидеть и нажать саму кнопку — на практике это медленнее и заметнее, чем кнопки громкости, которые срабатывают вслепую, без единого касания экрана. Можно включить тот же фейковый экран вместо мгновенного закрытия приложения — настраивается отдельно."
...
private val en10 = object : IStr10 {
    ...
    override val guidePanicButtonDesc = "A third, separate emergency-deletion method (besides the duress password and the volume buttons) — a persistent notification with a \"Delete everything\" action. Works from the lock screen too, but not instantly: you need to expand the notification to see and tap the button — in practice slower and more visible than the volume buttons, which trigger blind, with zero screen interaction. Can be set to show the same fake screen instead of just closing the app — configured separately."

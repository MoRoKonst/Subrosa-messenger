...
@Composable
fun ServerTotpSettingsScreen(onBack: () -> Unit) {
    ...
    var enabled by remember { mutableStateOf(TotpManager.isServerEnabled(context)) }
    ...
    DisposableEffect(messengerService) {
        ...
        svc?.onTotpSetupResult = { success, reason ->
            ...
            if (success) {
                ...
                if (secret != null) TotpManager.enableServerLocally(context, secret)
                enabled = true
                pendingSecret = null
                codeInput = ""
                message = s.serverTotpSetupSuccess
                isError = false
            } else {
                message = s.serverTotpErrSetupFailed + (reason?.let { " ($it)" } ?: "")
                isError = true
            }
        }
        svc?.onTotpDisableResult = { success ->
            busy = false
            if (success) {
                TotpManager.disableServerLocally(context)
                enabled = false
                disableCodeInput = ""
                message = s.serverTotpDisableSuccess
                isError = false
            } else {
                message = s.serverTotpErrDisableFailed
                isError = true
            }
        }
        onDispose {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(s.serverTotpTitle, fontWeight = FontWeight.Bold, fontSize = 18.sp) },
                navigationIcon = {
                    ...
                    Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        if (enabled) {
                            OutlinedTextField(
                                ...
                                label = { Text(s.serverTotpDisableCodeLabel) },
                                singleLine = true,
                                modifier = Modifier.fillMaxWidth()
                            )
                            Button(
                                ...
                                Text(s.serverTotpDisableButton, color = c.textPrimary)
                            }

...
- **Group call signaling (audio/video call setup for group calls) is not anonymized from the server operator**, by deliberate design, to keep calls reliable — the operator can see who is calling whom in a group call and when, though not the call's audio/video content. One-to-one (1:1) call signaling, text messages, images, files, and group text messages are anonymized from the server once an anonymous token pool is established with a contact, subject to the residual limitations documented in `SECURITY.md` (including that the anti-fingerprinting mechanism used when fetching a contact's prekey bundle discloses the requester's contact list to the server as decoy padding).
...
- **If the Client's deployment has Firebase Cloud Messaging (FCM) configured** (an optional, server-side setting used to wake the Android app for background push delivery — see `SECURITY.md`), Google, as a third party the Provider does not control, will receive a recipient's FCM token and the timing of push notifications sent to that device. FCM never carries message content. This setting can be left unconfigured if the Client's policy requires no third-party involvement at all, at the cost of less reliable background delivery.
...
- If the Client selects a Managed plan, the Provider will have operational access to the Client's infrastructure as described in Section 3 and, as a consequence of the group-call-signaling limitation above, will be able to observe group-call metadata (though not call content or the content of any other message type, and not 1:1 call metadata, which is anonymized the same as text) for group calls made through that deployment.

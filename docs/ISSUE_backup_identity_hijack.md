...
5. **Групповой ростер не рассылается при добавлении участника** —
   САМАЯ ВАЖНАЯ находка этого раунда, ломает реальную переписку в любой
   группе от 3 человек. Два изменения вместе: (а) включить подписанный
   `members`/`admins` в сам пакет `group_create`, чтобы получатель сразу
   инициализировал верный ростер; (б) при добавлении участника —
   рассылать уведомление всем существующим, не только новичку
   (симметрично уже правильно работающему удалению). Нужно менять формат
   пакета — затрагивает обе платформы, если Desktop имеет свою копию
   групповой логики (не проверено, есть ли там та же архитектура групп
   вообще — Desktop, возможно, не поддерживает группы совсем, стоит
   сначала проверить).
...
15. ~~**TOTP 2FA на бэкап**~~ — **MVP сделан, Android-only**. Новый
    ...
    **Второй, отдельный слой — server-side registration TOTP, добавлен в
    той же сессии, по прямому уточнению пользователя**: изначальная
    формулировка задачи в этом файле ("файл+пароль бэкапа") оказалась
    только половиной замысла — пользователь уточнил, что TOTP должен
    также (не вместо) защищать сам **`register` на сервере**, привязан
    к аккаунту (fingerprint), и что должен быть **ровно один секрет на
    аккаунт, без возможности выпустить второй** (иначе атакующий,
    ненадолго получивший контроль, мог бы сам себе довыпустить второй
    "легитимный" фактор и закрепиться).

    Реализовано:
    - `server.py`: новая таблица SQLite `user_totp` (username → secret,
      персистентно, переживает рестарт), `user_totp_secrets` в памяти для
      быстрой проверки, `user_totp_last_counter` — защита от replay
      (один и тот же 30-секундный код нельзя предъявить дважды).
      `totp_setup` — одноразовая привязка секрета к аккаунту: если для
      username уже есть секрет, сервер **безусловно отказывает**
      (`totp_setup_failed: already_enabled`), никакого пути перезаписи
      через API нет вообще, даже с валидной подписью текущего приватного
      ключа. `totp_disable` требует **текущий валидный TOTP-код**, не
      только принадлежность аккаунту — значит только приватного ключа
      (== только файла бэкапа + пароля) недостаточно, чтобы снять защиту.
      В `register` — если для username есть секрет, обязателен
      корректный `totp_code` в самом пакете `register`, проверяется **до**
      того, как трогается существующая сессия (`clients[username]`), так
      что неудачная/отсутствующая попытка кода никогда не вытесняет
      легитимно подключённого владельца — сервер просто отвечает
      `totp_required` и не регистрирует.
    - Android: `TotpManager.kt` расширен отдельным неймспейсом
      (`server_totp_prefs`, свой секрет, свой `isServerEnabled`) —
      сознательно не переиспользует backup-TOTP секрет/состояние, это
      две независимые защиты с разным порогом отключения (backup-TOTP
      выключается мгновенно локально, server-TOTP — только текущим кодом
      через сервер). `MessengerService.kt`: `register` теперь кладёт
      `totp_code` если `TotpManager.getServerSecret()` не null; новые
      исходящие `sendTotpSetup()`/`sendTotpDisable()`; обработчики
      `totp_setup_ok/failed`, `totp_disable_ok/failed` дергают колбэки
      для UI; `totp_required` логируется один раз за коннект
      (`totpRequiredNotified`), не заспамливает лог/уведомления при
      естественном цикле реконнектов. Новый экран
      `ServerTotpSettingsScreen.kt` (Profile → 🔒 TOTP-защита входа на
      сервер) — тот же enrollment-UX, что у backup-TOTP (секрет +
      `otpauth://` + подтверждение кодом), плюс отдельная форма
      выключения, требующая текущего кода.

    **Не сделано**: полноценный "введите код, чтобы завершить этот
    конкретный отложенный вход" флоу при `totp_required` — сейчас это
    просто лог + однократное уведомление в логе, реальный UI на этот
    случай не построен (упомянутый выше тупик "устройство без секрета"
    актуален и здесь: если это восстановленный бэкап без server-TOTP
    секрета, вход с этого устройства не завершится, пока пользователь
    вручную не привезёт секрет с исходного устройства — секрет по тем же
    причинам не кладётся ни в бэкап, ни куда-либо ещё). Desktop не
    тронут. Live-тест (два устройства, реальный сервер, реальный
    authenticator) не проводился — только синтаксис Python
    (`ast.parse`) и `compileDebugKotlin` зелёные.

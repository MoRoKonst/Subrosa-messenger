...
## Найдено вживую 2026-08-17 — доставка anon_message считалась завершённой без реального подтверждения получателем

Живьём, с `DEBUG-TOKENLIFE`-логами: сообщение с телефона не дошло до
эмулятора. Сервер при этом честно писал `Доставлено по токену …c6c028`
— **ровно в ту же секунду**, когда у эмулятора в логе `Handshake
failed`. Проверено: пользователь подтвердил, что это не редкий случай
("реально частый, потому что связь и так нестабильна... я
подключусь, он напишет новое подключение, увидит что старое упало и
всё, связь сломана") — то есть окно гонки бьёт достаточно часто, чтобы
быть основной, не краевой причиной потерь.

**Причина**: `server.py`'s `send_safe()` —
...
`True` означает только "запись в сокет не бросила исключение" — не
"получатель реально обработал сообщение". Если соединение получателя
уже фактически разваливается (а именно так и было — `Handshake
failed` в ту же секунду), `ws.send()` вполне может отработать
"успешно" на уровне библиотеки, а байты до приложения так и не
дойдут. При этом `anon_message`-хендлер **сразу** после успешного
`send_safe()` помечал токен потраченным (`token_to_ws.pop`) — никакого
дальнейшего пути для этого сообщения не оставалось вообще, ни "офлайн,
в очередь", ничего.

**Исправлено — реальное end-to-end подтверждение**:
- Перед попыткой живой доставки сообщение теперь **всегда** кладётся в
  `token_pending[token]` (одна запись — токены одноразовые, больше
  одной там в принципе быть не может) — это и есть страховочная копия.
- `send_safe()`-успех больше не считается финалом — токен не
  выкидывается из `token_to_ws` сразу.
- Клиент, доставке которого пришёл `anon_delivery` (теперь с полем
  `token` в конверте, не только `payload`), после **реальной**
  обработки (`handleMessage()` отработал без исключения) шлёт новый тип
  сообщения `anon_delivery_ack` с этим токеном.
- Сервер по получении `anon_delivery_ack` окончательно чистит
  `token_pending`/`token_to_ws`/`ws_to_tokens` — только теперь токен
  действительно "потрачен".
- Если ACK не пришёл (ровно случай из живого теста) — специальный
  ретрай не нужен: уже существующий флаш `token_pending` при
  `subscribe_tokens` (срабатывает на любом следующем реконнекте)
  естественным образом передоставит то же сообщение. Цена — редкий
  безобидный дубль вместо тихой потери; дедупликация на клиенте
  (`receivedMessageIds`/`REPLAY_WINDOW_MS`) уже существует для обычных
  сообщений.

**Desktop-клиент** (`WebSocketClient.kt`, не в гите) получил
зеркальный фикс — иначе он никогда не шлёт `anon_delivery_ack`, и
сервер держал бы для него всё в вечно неподтверждённом
`token_pending`, передоставляя всё заново на каждом будущем
реконнекте (дубли, не потери, но всё равно неверно). Не собирался и не
тестировался этим проходом.

**Не сделано**: TTL/чистка для `token_pending`-записей, чей ACK так и
не пришёл никогда (аккаунт удалён/отозван/навсегда офлайн) — тот же
класс компромисса, что уже принят для `known_tokens` выше в этом
файле, рост ограничен количеством когда-либо реально отправленных (не
просто выданных) сообщений на аккаунт, не трафиком. Побочно замечено,
не исправлено: поле `_anon_token` в `anon_delivery`-конверте читается
клиентом (`AnonTokenManager.consumeMyToken`), но нигде и никогда не
записывается — ни клиентом, ни сервером — то есть эта локальная чистка
собственного пула токенов никогда не срабатывает; собственный список
"я раздал" растёт только через `ensureMyTokenPool`, никогда не
уменьшается по факту потребления. Отдельный, не толкнутый в этом
проходе гэп.

`py_compile` (сервер) и `compileDebugKotlin` (Android) зелёные. Не
задеплоено, не протестировано вживую.

**Влияние на безопасность/анонимность — пользователь спросил прямо,
зафиксировано здесь**. Свойства, которые не меняются:
- Сервер по-прежнему не может связать отправителя с получателем —
  схема адресации по одноразовому токену не менялась, изменилось
  только то, в какой момент сервер считает конкретный токен
  "потраченным".
- `payload` как был end-to-end зашифрован (гибрид classic+PQ или
  Double Ratchet), так и остался — сервер как видел только шифротекст,
  так и видит.
- Токен, который теперь передаётся получателю в конверте
  `anon_delivery` (раньше передавался только `payload`), не раскрывает
  ничего нового — это тот же токен, который получатель сам когда-то
  сгенерировал и отдал отправителю через `ensureMyTokenPool`/
  `sendAnonTokensTo`; увидеть его снова — не утечка.
- `anon_delivery_ack` с этим токеном ничего не говорит серверу об
  отправителе — сервер и раньше не знал, кто отправитель, анонимность
  тут обеспечивается самой схемой адресации, а не подтверждением
  доставки.

Единственный реальный, но количественный (не новый по классу)
компромисс: раньше при успешном `send_safe()` шифротекст сразу
забывался сервером; теперь он лежит в оперативной памяти сервера
(`token_pending`, не на диске) чуть дольше — до получения ACK, в норме
доли секунды. Это ровно та же модель хранения, что уже была для случая
"получатель офлайн" (там шифротекст и раньше лежал в очереди) —
изменилось только то, что теперь она применяется единообразно ко всем
случаям, а не только к заведомо-офлайн. Контент остаётся зашифрованным
всё это время, так что это не новый класс утечки — просто немного
шире окно "непрочитанный шифротекст лежит в памяти сервера", а не
раскрытие содержимого.

## Найдено вживую 2026-08-17 (тот же день, сразу после первого деплоя ACK-фикса) — ACK слал подтверждение, даже если сообщение не расшифровалось

Живьём, сразу после выката предыдущего фикса: сервер честно показывал
`Отправлено по токену …cb5d99 (ждём anon_delivery_ack)`, затем
`Подтверждена доставка токена …cb5d99` — то есть ACK **пришёл**. Но у
получателя в логе нет строки `ПОЛУЧЕНО message ...` вообще — сообщение
реально не расшифровалось и не показалось пользователю. Пользователь
поймал это сразу: "ты будто логи не читаешь до конца... опять есть
отправлено, но нет получено".

**Причина**: условие для отправки `anon_delivery_ack` в клиенте —
"`handleMessage()` отработал без исключения". Но обработчик `"message"`
в `handleMessage()` (и аналогично `session_init`/`voice`) **ловит
ошибку расшифровки сам, внутри себя**, делает восстановление
(`session_reset`, `requestPrekeyBundle`) и **не пробрасывает исключение
дальше** — то есть с точки зрения вызывающего кода `handleMessage()`
"успешно завершился", хотя реально ничего не расшифровалось и не
показалось. Мой ACK ориентировался ровно на этот неверный сигнал.

Ровно тот же класс, что и вчерашняя "OPK N уже использован" — только
вчера это приводило к молчаливой потере из-за старого проглатывания
исключения, а сегодня ACK-фикс превратил ту же самую проглоченную
ошибку в **ложное подтверждение доставки**, что даже хуже: теперь
сервер уверен, что всё дошло, и `token_pending`-страховка уже вычищена
— второй попытки не будет вообще.

**Исправлено**: в трёх местах, где раньше `catch` только логировал и
восстанавливался (`"message"`-хендлер, оба catch-блока —
no-session-фоллбек и основной; `processSessionInit()`; `"voice"`) —
теперь после тех же самых действий по восстановлению исключение
**дополнительно перебрасывается** (`throw e`). Ни один внешний
вызывающий код это не проглатывает — оба места, откуда зовётся
`handleMessage()` (`onMessage`-диспетчер WS и новый `anon_delivery`-
хендлер), уже оборачивают вызов в свой `try/catch`, который просто
логирует и (для `anon_delivery`) корректно **не шлёт ACK**.

**Desktop** (`WebSocketClient.kt`, не в гите) получил зеркальный
фикс — там `runCatching { }.getOrElse { ...; return }` заменено на
`throw it` в тех же двух местах `"message"`-обработчика.

**Не сделано — важно**: это исправлено только для трёх конкретных
подтверждённых/вероятных случаев. `handleMessage()` — огромная функция
с ~60-80 блоками `catch`, у многих из которых, вероятно, тот же паттерн
"залогировать и тихо восстановиться без re-throw" (`group_message`,
image/file/video chunks, реакции и т.д. — не проверено поштучно).
Системный аудит всех веток на этот же класс проблемы не проводился в
этом проходе — это отдельная, более крупная задача.

Compiles clean (`compileDebugKotlin`, Desktop не собирался). Не
задеплоено, не протестировано вживую.

## Найдено вживую 2026-08-17 (тот же день, третий проход) — флаш при subscribe_tokens не учитывал новую ACK-модель вообще

Живьём, сразу после деплоя предыдущих двух фиксов: сообщение
(`4b5a8549`, токен `…819be9`) сервер честно отправил ("ждём
anon_delivery_ack"), но подтверждения не последовало никогда, и
**повторной попытки тоже не было** — сообщение пропало насовсем.
Пользователь поймал это по асимметрии ("от телефона связь есть, до
телефона нет") и явно указал: "сообщение которое я отправил в момент
дисконнекта не дошло и потеряно".

**Причина**: ровно тот же класс бага, что чинили в основном
`anon_message`-хендлере — только забыл применить ту же дисциплину во
**втором** месте, где `token_pending` читается: флаш при
`subscribe_tokens` (срабатывает на каждом реконнекте):

```python
pending = token_pending.pop(t, [])          # ← безусловно выкидывает
for p in pending:
    asyncio.create_task(send_safe(websocket, json.dumps(p)))  # ← результат нигде не проверяется
```

Запись выкидывалась из очереди **до** попытки повторной доставки, сама
попытка — `asyncio.create_task`, результат которой никто и никогда не
проверял и не использовал. Если эта повторная попытка тоже не
доходила до получателя (а тут как раз это и произошло — у эмулятора в
этот момент шла своя пересборка сессии с сопутствующими decrypt-
ошибками, см. предыдущую находку) — сообщение исчезало безвозвратно,
без единого следа где-либо.

**Исправлено**: `token_pending.pop(t, [])` → `token_pending.get(t, [])`
— запись больше не выкидывается на этом шаге. Теперь единственное
место, где `token_pending` окончательно чистится — обработчик
`anon_delivery_ack` (см. предыдущую находку). Если повторная попытка
при флаше снова не дойдёт — запись останется в очереди и передоставится
на **следующем** реконнекте, а не пропадёт.

`py_compile` зелёный. Не задеплоено, не протестировано вживую.

## Найдено вживую 2026-08-17 (тот же день, четвёртый проход) — настоящий корень всей сессионной чехарды: сессии стирались на каждом реконнекте

Пользователь прямо спросил: "сообщение он не восстанавливает, его
просто не существует дальше" — и это меняет диагноз. Проблема не только
в доставке байт: даже когда сообщение технически передоставляется через
`token_pending`, оно закодировано под ratchet-состояние, которого у
получателя уже физически нет — переигрывание тех же байт бесполезно
навсегда, если сессия удалена.

Нашёл источник — совсем не то место, что гейтилось раньше сегодня
(`sessionManagerInitialized`, которое про повторный вызов
`initialize()` при получении `handshake_ok`). Это **отдельный,
более ранний** вызов в `connect()`, сразу после отправки `register`,
**без всякого условия**:
...
`deleteAllSessions()` — не косметика: она чистит Double Ratchet-сессии
**со всеми контактами** и в памяти, и **на диске**
(`encPrefs...filter { it.startsWith("session_") }.forEach { editor.remove(it) }`),
плюс OPK-пул и текущий SPK в памяти. Это выполняется абсолютно на
**каждом** успешном реконнекте — не только при первом запуске
приложения, а при любом сетевом моргании, DNS-сбое, чём угодно,
приводящем к новому `connect()`.

Это и есть корень всей вчерашне-сегодняшней чехарды:
- Вчерашний "OPK N уже использован" — OPK-пул подчищался и
  перезагружался на каждом реконнекте, создавая окна рассинхрона.
- Сегодняшние повторяющиеся "Пакет повреждён (ephemeral key)" +
  взаимные `session_reset` — сессия с контактом физически исчезала
  прямо в момент реконнекта, поэтому расшифровка входящего (закодированного
  под старую, ещё живую на момент отправки сессию) закономерно
  проваливалась.
- Постоянные `isFirst=true`/"X3DH сессия ... инициирована" сразу после
  реконнектов — потому что сессия объективно исчезала, а не "казалась
  потерянной".

**Исправлено**: обе строчки убраны из `connect()`. `initialize()` и так
уже вызывается ровно один раз за жизнь процесса в другом месте (гейт
`sessionManagerInitialized`, сегодняшний более ранний фикс) —
предполагаю, что это дублирующий, ранний, но так и не защищённый
guard'ом код из того же исходного замысла "инициализировать один раз",
просто (в отличие от второго места) ещё и **разрушал** состояние
вместо того, чтобы просто перечитать его. Осознанные вызовы
`deleteAllSessions()` в `BackupManager.kt` (сброс identity) и
`WipeManager.kt` (панич-вайп) не тронуты — это правильные, точечные
использования. Desktop (`WebSocketClient.kt`, не в гите) — такого
паттерна там нет, проверено, не трогалось.

**Не восстановимо**: сообщения, уже потерянные из-за этого бага
(закодированные под уже стёртую сессию) — восстановить нельзя в
принципе, это не баг доставки, а свойство forward secrecy (стёртый
ratchet-ключ не восстанавливается никак, это и есть весь смысл forward
secrecy). Начиная с этого фикса подобные потери прекратятся, но
задним числом ничего не вернуть.

Compiles clean (`compileDebugKotlin`). Не задеплоено, не протестировано
вживую — нужно повторить тот же сценарий (реконнект/потеря сети) и
убедиться, что сессия с контактом переживает реконнект, и "Пакет
повреждён"/`session_reset`-цикл больше не появляется просто от факта
переподключения.
...
## Найдено вживую 2026-08-17 (пятый проход) — `token_to_ws` стирался зомби-соединением уже после того, как токен переподписан на новое

После фикса из четвёртого прохода (`deleteAllSessions()` больше не рвёт
сессии на каждом реконнекте) добавлены парные логи на отправку/приём
квитанций `delivered`/`read` (`ОТПРАВЛЯЮ квитанцию`/`ПОЛУЧЕНА квитанция`,
[MessengerService.kt](../app/src/main/java/com/example/test/MessengerService.kt))
для дальнейшей диагностики.

Живой лог показал: устройство B (`643A925F62FDDC51`) переподключилось,
успешно сделало `subscribe_tokens` на 50 токенов в 03:55:30.7 (серверное
время), но спустя 18-42 секунды после этого сервер начал репортить те же
самые токены (`…61fc23`, `…63c050`, `…f8809f`, `…77bcbb`) как "офлайн",
хотя B оставался подключён и активно работал всё это время (успешные
`pollMailbox`, отправка/приём сообщений). Одно сообщение
(`61a5dc38...`, отправлено через токен `…63c050`) было безвозвратно
потеряно этим путём — `sendWs=true` на отправителе, но никогда не дошло
до получателя и не всплыло дальше в логе.

**Причина**: в `server.py`, `finally`-блок дисконнекта (после
`known_tokens`/`token_pending` фикса от 2026-08-13):
```python
owned_tokens = ws_to_tokens.pop(websocket, set())
for t in owned_tokens:
    token_to_ws.pop(t, None)
```
Клиент на каждом реконнекте переподписывается на один и тот же
персистентный набор ~50 токенов. Если СТАРОЕ соединение — зомби (TCP
полуоткрыт, сервер узнаёт об этом только по истечении
`ping_timeout=30с`), а клиент уже успел переподключиться и заново
подписаться на НОВОМ сокете в этом окне, `token_to_ws[t]` на момент
срабатывания этой очистки уже корректно указывает на новое соединение —
но `pop(t, None)` безусловный, не проверяет, что запись всё ещё
принадлежит именно умирающему (старому) сокету, и стирает её. Тот же
класс бага, что уже чинили для `known_tokens` (2026-08-13) и для флаша
`token_pending` при `subscribe_tokens` (тот же день, третий проход выше)
— третье и, вероятно, последнее место с этим паттерном на карте токенов.

**Исправлено**: добавлена проверка владения перед удалением —
```python
owned_tokens = ws_to_tokens.pop(websocket, set())
for t in owned_tokens:
    if token_to_ws.get(t) == websocket:
        token_to_ws.pop(t, None)
```
Если запись уже переписана более новым соединением — она остаётся
нетронутой; удаляется только если она всё ещё указывает на тот сокет,
который только что умер.

`py_compile` чисто. Не задеплоено, не протестировано вживую — нужно
повторить тот же сценарий (быстрый реконнект + отправка в первые ~30с
после него) и по серверному логу убедиться, что токены из свежего
`subscribe_tokens` больше не репортятся "офлайн" в течение
`ping_timeout`-окна после подписки.

## Найдено вживую 2026-08-17 (шестой проход) — фикс из пятого прохода не сработал: в `finally` было ДВЕ копии одной и той же безусловной очистки

Пользователь задеплоил только сервер (без пересборки клиентов), сразу
стало ощутимо лучше, но контрольный тест с обрывом связи снова
воспроизвёл ту же картину. Свежий лог показал `[RECONNECT]
643A925F62FDDC51 переподключился` в 04:06:27 (сервер), `[ANON]
643A925F62FDDC51 подписан на 50 токенов` в 04:06:28 — и только затем, в
04:06:33, `[-] Отключился: 127.0.0.1` (запоздалое закрытие СТАРОГО
сокета — `existing["ws"].close()` из `register`-хендлера при обнаружении
дубликата, `server.py:1475`, асинхронная задача, реально выполняется с
задержкой). Через 6-31 секунду после этого те же самые только что
переподписанные токены (`…cc477b`, `…cb5d99`, `…93c714`, `…819be9`)
снова "офлайн" — то есть фикс из пятого прохода не сработал.

**Причина**: в `finally`-блоке `handle_client()` оказалось **два**
отдельных места, делающих одно и то же — очистка `ws_to_tokens`/
`token_to_ws` для дисконнектнувшегося сокета. Более раннее (по
расположению в коде, значит и по порядку выполнения) — без единого
комментария, явно случайный дубль, оставшийся от более старой версии
этого куска логики:
```python
...
for t in tokens:
    token_to_ws.pop(t, None)
```
Оно безусловное — никакой проверки владения. И оно выполняется **первым**
в `finally`, поэтому к моменту, когда исполнение доходит до второго,
уже исправленного блока (тот, что с проверкой `token_to_ws.get(t) ==
websocket`, добавленной в пятом проходе), `ws_to_tokens.pop(websocket,
set())` там уже возвращает пустой набор — весь набор токенов уже выгребен
и стёрт первым блоком. Фикс из пятого прохода был технически корректным,
но фактически мёртвым кодом — просто никогда не получал данные для
обработки.

**Исправлено**: первый (безусловный, бескомментарийный) блок удалён
целиком, оставлен только второй (с проверкой владения). Теперь очистка
`token_to_ws` для дисконнектнувшегося сокета происходит ровно один раз, и
именно тем путём, который проверяет, что запись всё ещё принадлежит
именно этому (умирающему) сокету.

`py_compile` чисто. Не задеплоено, не протестировано вживую — нужен тот
же сценарий (быстрый реконнект + активная переписка сразу после), и в
этот раз — убедиться, что "[-] Отключился" от старого сокета,
приходящий уже ПОСЛЕ новой подписки, больше не сопровождается "офлайн"
для только что переподписанных токенов.

## Найдено вживую 2026-08-17 (седьмой проход) — сессии рвались в бесконечном цикле после единичного decrypt-сбоя: session_reset слался без дебаунса

После деплоя фикса из шестого прохода пользователь протестировал обрыв
связи заново — токен-баг ушёл, но проявилась другая, отдельная проблема:
устройство B (`643A925F62FDDC51`) поймало `BAD_DECRYPT` от A
(`FC2FF837F828F392`), и дальше пошёл самоподдерживающийся цикл на ~40+
секунд: повторяющиеся "Ошибка расшифровки… Пакет повреждён (ephemeral
key)", `session_reset` туда-сюда, `Prekey bundle republish по запросу
сервера` (сервер видит, что у A быстро тает пул OPK — верный признак, что
X3DH пытаются провести заново много раз подряд). В логе B видно **пять**
попыток отправить `session_reset` контакту A в пределах ~3.5 секунд
(`поставлено в очередь (в очереди: 1..5)`, все застряли в очереди, пока
у B не было живого токена/соединения). В логе A — соответствующие
`session_reset от 643A925F62FDDC51 — сбрасываем сессию`, причём дважды
почти одновременно (`12:00.214` и `12:00.218`), и затем ещё раз тем же
парным образом на `12:20.059`/`12:20.427`.

**Причина**: catch-блоки на decrypt-ошибку (`"message"`-хендлер, оба —
и inner no-session fallback, и outer; плюс `processSessionInit()`) при
**каждом отдельном** провалившемся сообщении безусловно делают полный
"тройной" ресет — `SessionKeyManager.deleteSession(from)` +
`requestPrekeyBundle(from)` + отправка `session_reset` — без какого-либо
дебаунса. Если несколько сообщений от одного контакта проваливают
расшифровку подряд (что при нестабильной связи — почти гарантия:
несколько отправленных/переотправленных сообщений закодированы под уже
несовпадающее ratchet-состояние), каждое из них независимо шлёт СВОЙ
`session_reset`. Полученные с задержкой (через очередь/mailbox, пока
отправитель был офлайн) дубликаты `session_reset` продолжают приходить
уже ПОСЛЕ того, как получатель успел заново установить рабочую сессию по
первому ресету — и каждый лишний дубликат сносит эту свежую, уже рабочую
сессию заново. Цикл не затухает сам по себе, потому что каждый новый
провал расшифровки (в том числе спровоцированный предыдущим лишним
ресетом) порождает ещё один ресет.

**Исправлено**: новый `sendSessionResetDebounced(to)` в
`MessengerService.kt` — тот же паттерн, что уже применён к `sendTyping()`
(`lastTypingSentAt`), но с окном 10с вместо 3с. Оборачивает всю "тройку"
(`deleteSession` + `requestPrekeyBundle` + отправка `session_reset`) и
не даёт её повторить для того же контакта чаще одного раза в 10 секунд.
Заменены все три места, инициирующие ресет по факту decrypt-ошибки (два
catch-блока в `"message"`, один в `processSessionInit()`). Явный
пользовательский триггер (intent-extra `send_session_reset_to`) и
батч-рассылка после восстановления бэкапа (`pendingResetContacts`) —
**не тронуты**: это редкие, осмысленные, не decrypt-failure-driven вызовы,
дебаунс им не нужен и мог бы помешать (например, ресет сразу нескольким
контактам после импорта бэкапа не должен зависеть от таймера одного
контакта).

Compiles clean (`compileDebugKotlin`). Не задеплоено, не протестировано
вживую — нужно повторить тот же обрыв-сценарий и убедиться, что после
единичного decrypt-сбоя цикл `session_reset`/`Пакет повреждён` больше не
тянется десятками секунд, а гасится за один обмен.

## Найдено вживую 2026-08-17 (восьмой проход) — гонка потоков: deleteSession() не синхронизирован с nextSendKey/nextRecvKey

Пользователь подтвердил: короткий разрыв + пара сообщений — работает
нормально; **длинный** разрыв + много накопившихся сообщений — снова
проблема. Свежий лог: после реконнекта на устройство прилетает пачка из
~8 сообщений от одного контакта почти одновременно (разные потоки
`Dispatchers.IO`, все временные метки в пределах одной миллисекунды).
Первое — `BAD_DECRYPT`. Следующие семь — уже не `BAD_DECRYPT`, а **`Нет
сессии с FC2FF837F828F392`** (`IllegalStateException` из
`SessionKeyManager.kt:718`, `nextRecvKey()`), хотя секунду назад сессия
точно была (иначе первое сообщение не дошло бы до стадии decrypt вообще).

**Причина**: `nextSendKey()`/`nextRecvKey()` в `SessionKeyManager.kt`
обёрнуты в `synchronized(this)` — это специально сделано, чтобы
параллельные decrypt-вызовы для одного контакта не гонялись за один и тот
же chain key. Но `deleteSession()`/`deleteAllSessions()` **не были
синхронизированы тем же монитором** — просто `sessions.remove(...)` /
`sessions.clear()` напрямую. `ConcurrentHashMap` делает сами эти вызовы
потокобезопасными по отдельности, но не защищает от гонки с уже идущим
`nextRecvKey()` для того же контакта: пока сообщение #1 (провалившее
decrypt) вызывает `sendSessionResetDebounced()` → `deleteSession(from)`
на одном потоке, сообщения #2-#8 из той же пачки могли к этому моменту
уже находиться внутри своего собственного `synchronized(this)`-блока
`nextRecvKey()` на других потоках, ожидая своей очереди на монитор.
Поскольку `deleteSession()` **не берёт тот же монитор**, он не ждёт и
сносит запись сессии в произвольный момент между вызовами `nextRecvKey()`
для соседних сообщений той же пачки — следующее сообщение, зайдя в свой
`synchronized`-блок, находит `sessions[contactId] == null` и падает с
"Нет сессии", хотя расшифровать его теоретически было можно (сессия была
жива до того, как её сорвало гонкой).

Раньше это было не так заметно: пары сообщений почти никогда не создавали
достаточно узкое окно гонки. Пачка из многих сообщений сразу после
долгого разрыва — намного более вероятный триггер, что и подтверждает
наблюдение пользователя.

**Исправлено**: `deleteSession()` и `deleteAllSessions()` теперь тоже
`synchronized(this)` — тот же монитор, что и у `nextSendKey`/
`nextRecvKey`. Теперь удаление сессии либо полностью происходит ДО
начала decrypt-попытки соседнего сообщения из той же пачки, либо
полностью ПОСЛЕ неё — никогда не посередине.

Compiles clean (`compileDebugKotlin`). Не задеплоено, не протестировано
вживую — нужен тот же сценарий (длинный разрыв, много накопившихся
сообщений одному контакту) и проверка, что пачка либо вся расшифровывается
успешно, либо валится единым, согласованным `session_reset`-циклом — без
части сообщений, падающих отдельно с "Нет сессии" из-за гонки.

## Инструментация 2026-08-17 (девятый проход) — счётчики ratchet в лог, для диагностики "3 из 9 дошли"

После фикса из восьмого прохода живой тест дал 3 из 9: три успеха
(включая один явный `Использован пропущенный ключ` — механизм
skipped-keys реально сработал и подтвердил, что `synchronized`-фикс
корректно сериализует `deleteSession()` относительно `nextRecvKey()`),
но три сообщения (не считая трёх, чья потеря уже объяснена forward
secrecy — переотправка стале-шифротекста поверх новой сессии) упали с
чистым `BAD_DECRYPT` без видимой причины в существующих логах.

Гипотеза, не подтверждённая: возможный рассинхрон `epochPrefix` между
моментом, когда пропущенные ключи для более ранних counter'ов
СОХРАНЯЮТСЯ (при обработке более позднего, более "далёкого" по счётчику
сообщения) и моментом, когда более ранние сообщения приходят позже и
пытаются их ИСПОЛЬЗОВАТЬ — если между этими двумя точками произошёл DH
ratchet step, `lookupKey` для одного и того же логического counter'а
может отличаться, и код молча проваливается в "нормальную" ветку
(строка ~822 в `nextRecvKey()`), выводя **неправильный** ключ вместо
ошибки — что и выглядит как voiceless `BAD_DECRYPT` на стороне вызова
AEAD.

**Добавлено (только логирование, без изменения поведения)**:
`SessionKeyManager.kt`, `nextRecvKey()` — при каждом вызове лог
`DEBUG-RATCHET nextRecvKey(...)`: `expectedCounter`, `recvCounter`,
`isDHStep`, `dhKeyB64`, `currentDhPubB64`, `lookupKey`, полный набор
ключей `skippedKeys`. Отдельный `Log.w` в ветке, где `expectedCounter <=
recvCounter`, но пропущенного ключа под `lookupKey` не нашлось (именно
эта ветка подозревается — она молча трактует такой случай как "текущее
по порядку сообщение" без проверки, что `expectedCounter == recvCounter`
на самом деле).

Compiles clean (`compileDebugKotlin`). Ждём следующий раунд того же
теста (длинный разрыв, много сообщений одному контакту) — по новым
логам сразу будет видно, срабатывает ли именно эта ветка перед
провалившимися `BAD_DECRYPT`, и что там за `lookupKey`/`skippedKeys` в
этот момент.

**Результат следующего раунда: 10 из 10 доставлено.** Тот же сценарий
(долгий разрыв, пачка сообщений одному контакту), с диагностическим
логированием включённым — ни одной потери, `DEBUG-RATCHET`-предупреждение
ни разу не сработало. Похоже, частичная потеря (3 из 9) в восьмом
проходе была последним отголоском самой гонки потоков
(`deleteSession()`/`nextRecvKey()`), а не отдельным багом в
epoch-prefix/skipped-keys логике — synchronized-фикс закрыл проблему
полностью. Диагностические логи оставлены в коде на будущее (не мешают,
включены на `Log.d`/`Log.w`), гипотезу про рассинхрон `epochPrefix`
считаем неподтверждённой и низкоприоритетной — заново поднимать только
если похожая потеря повторится в логах с этим инструментированием.

## Найдено 2026-08-17 — "Удалить у всех" молча теряла сигнал офлайн, плюс кнопка "Отправить снова"

Пользователь спросил, что реально делает кнопка "Удалить у всех". Разбор
показал: сама логика верная и безопасная — `message_delete`
подписывается (`CryptoManager.sign(messageId)`), получатель проверяет
подпись перед тем, как удалить у себя ЗАПИСЬ ИЗ ХРАНИЛИЩА (не только из
UI-списка), так что подделать чужое удаление нельзя, и переоткрытие чата
не возвращает "удалённое у всех" сообщение.

**Но найден реальный баг**: `sendDeleteMessage()`/`sendGroupDeleteMessage()`
в `MessengerService.kt` начинались с `if (!isConnected) return` —
безусловный выход, если в момент нажатия кнопки нет соединения. В отличие
от обычной отправки текста (`send()`), здесь **не было** офлайн-фолбэка
вообще — при нажатии "Удалить у всех" оффлайн: у себя сообщение исчезает
сразу (это делается локально, до сетевого вызова), а вот собеседнику
сигнал не уходит вообще, не ставится в очередь, не ретраится — молча и
навсегда. Собеседник продолжает видеть "удалённое у всех" сообщение
бесконечно, без единого признака в UI, что что-то пошло не так.

**Исправлено**: убран `if (!isConnected) return` в обеих функциях —
`sendAnonOrDirect()`, куда оба вызова и так уходят, уже сам корректно
ставит пакет в очередь при отсутствии связи (`queuePendingAnon`), точно
так же, как это работает для обычных сообщений. Проверка была не просто
лишней, а вредной.

**Отдельно, по запросу пользователя**: добавлена кнопка "Отправить
снова" в контекстное меню сообщения — видна только для собственных
сообщений с `isFailed = true` (не доставленных за 60с, см. существующий
`deliveryTimeoutJob` у обычной отправки). Действие: удаляет застрявшее
сообщение (локально) и повторяет отправку с новым id — та же логика
pending/таймаут-на-fail, что у обычной отправки, только без переноса
`replyToId` на новый цикл вложенного reply (сохраняется, если было).
Не меняет то, как теряются оригинальные сообщения — просто даёт ручной
способ повторить попытку, не набирая текст заново.

Compiles clean (`compileDebugKotlin`). Не задеплоено/не протестировано
вживую.

## Найдено вживую 2026-08-17 — пароль экрана блокировки утёк в поле ввода чата (нет фокуса на password-поле)

Пользователь поймал редкий, но реальный кейс: обновил APK, открыл
мессенджер — клавиатура появилась сразу, но поле пароля на экране
блокировки визуально не было активным. Не заметив этого, ввёл пароль
"вслепую", затем поле активировалось, ввёл пароль второй раз (уже
реально сработавший) — а позже обнаружил свой первый ввод пароля
**в виде обычного текста в поле ввода сообщения одного из чатов**.

**Причина**: экран блокировки (`MainActivity.kt`, `lockVisible`-блок) —
это `AnimatedVisibility`-оверлей поверх уже скомпозированного экрана
(например, чата), не полноценная замена экрана. Compose не переносит
фокус клавиатуры на новый composable просто потому, что он стал видимым
— OutlinedTextField пароля не имел `FocusRequester` вообще. Если на
момент появления оверлея фокус (с прошлой сессии, до сворачивания/
рестарта) всё ещё держало какое-то текстовое поле экрана под оверлеем
(поле ввода сообщения), а клавиатура при этом видна (ОС может показать
её сразу после cold start/обновления) — все нажатия шли в то поле, не в
поле пароля. Пароль в открытом виде осел в черновике сообщения
(`ChatStorage`-драфт, если `inputText` успел сохраниться) — заметная
дыра, хоть и требующая довольно специфического стечения обстоятельств,
чтобы попасться живьём.

**Исправлено**: `FocusRequester` на поле пароля + `LaunchedEffect(lockVisible)`,
который при появлении оверлея сначала явно снимает фокус со всего
(`LocalFocusManager.clearFocus(force = true)`), затем (с задержкой 100мс,
чтобы поле успело встать в композицию/layout) явно запрашивает фокус на
себя. Теперь ввод при блокировке физически не может уйти ни в одно
другое поле.

**Не проверено**: `LoginScreen.kt` (первичный вход, не оверлей блокировки)
имеет тот же класс уязвимости в принципе — тоже нет `FocusRequester` на
поле пароля — но это полноценная замена экрана (`screen = "login"`), а
не оверлей поверх другого composable, так что риск "фокус остался на
чужом поле" там ниже (нечему держать фокус под ним). Не трогалось, не
подтверждено как реальная проблема — отдельная задача, если понадобится.

Compiles clean (`compileDebugKotlin`). Не задеплоено/не протестировано
вживую — нужно повторить похожий сценарий (открыть с уже видимой
клавиатурой сразу после запуска/обновления) и убедиться, что поле пароля
сразу активно.

## Добавлено 2026-08-17 — лимит регистраций и proof-of-work против Sybil-флуда с разных IP/устройств

Пользователь спросил про угрозу DoS/флуда, имея только адрес+порт
сервера. По коду и `nginx`-конфигу (`docs/nginx-subrosamessenger.conf.snippet`)
выяснено: внешние порты уже ограничены `allow <cloudflare>; deny all` —
прямой волюметрический DDoS по IP/порту не должен доходить до origin,
**если** этот конфиг реально задеплоен и Cloudflare-проксирование
включено (не проверено из репозитория, это внешняя настройка).

Отдельная, реальная угроза — Sybil-флуд регистраций: rate-limit
(`rate_limit_check`) считается по username, то есть по identity, а
identity бесплатно штампуется новым ECDSA-ключом на каждое соединение.
Атакующий с множеством устройств/IP обходит любой IP-based лимит
тривиально — заводит N identity вместо одной. Существующий
`SERVER_ACCESS_PROTECTED` (инвайт-код на первую регистрацию) уже
полностью закрывает эту угрозу, если включён, но по умолчанию выключен.

**Добавлено, оба выключены по умолчанию (0 = не активно)**:
- `MAX_REGISTERED_USERS` (env, счётчик по `registered_fingerprints`,
  переживает рестарт) — выше лимита новые регистрации отклоняются
  (`{"type": "registration_full"}`), уже зарегистрированные по-прежнему
  переподключаются без ограничений. Не число, зашитое в код — осознанно
  сделано настройкой (`os.environ`), у каждого деплоя своя ёмкость.
- `POW_DIFFICULTY_BITS` (env, по умолчанию 18 при выставлении) —
  первая-в-жизни регистрация нового fingerprint'а должна включать
  `pow_nonce`, такой что `sha256(handshake_challenge + pow_nonce)` имеет
  не меньше этого числа ведущих нулевых бит. Цена — реальное CPU-время
  устройства, не привязана к IP: у атакующего с 10000 устройств будет
  ровно 10000× реальных вычислений, а не одно IP-ограничение, которое он
  обходит сменой адреса. На реконнект уже известного fingerprint'а не
  проверяется вообще — платится ровно один раз за identity.

**Обе проверки — только для по-настоящему новых fingerprint'ов**
(`is_new_fingerprint`, вычисляется один раз, переиспользуется и для
существующей проверки `access_code`) — реконнекты существующих
пользователей не платят ни лимит, ни PoW, ни при каждом реконнекте не
дёргается лишний DB-запрос.

**Клиент** (`MessengerService.kt`): `challenge`-байты из handshake теперь
сохраняются (`lastChallengeBytes`), пакет `register` перед отправкой
тоже (`lastRegisterPacket`) — на случай, если сервер ответит
`pow_required`, клиент прозрачно для пользователя решает PoW
(`solveProofOfWork`, простой брутфорс nonce на `Dispatchers.Default`,
никакого UI) и повторяет **тот же самый** пакет `register` с добавленным
`pow_nonce` — без пересбора `recovery_code`/`access_code` (recovery-код
одноразовый, второй раз собрать было бы нечем). `registration_full` —
не транзиентная ошибка, retry не делается, просто Toast
(`serversRegistrationFull`, ru+en).

Compiles clean (`compileDebugKotlin`, `py_compile`). Не задеплоено, не
протестировано вживую — нужно включить оба флага на тестовом сервере,
завести новый аккаунт (проверить, что PoW реально решается и регистрация
проходит), выставить `MAX_REGISTERED_USERS` в 1 и убедиться, что второй
новый аккаунт получает `registration_full`, а уже существующий по-прежнему
подключается.

## Найдено вживую 2026-08-18 — видеозвонок под нагрузкой вызвал двойное использование одноразового токена

После видеозвонка у пользователя отправитель (телефон) продолжал считать,
что сообщения уходят (`sendWs=true`, валидный токен), но получатель
(эмулятор) их не видел. `=== СЕРВИС ЗАПУЩЕН ===`, повторявшийся несколько
раз подряд во время звонка — оказался красной селёдкой: это не краш
(`FATAL`/`AndroidRuntime`/`Fatal signal` в логе не найдено вообще), просто
сигналинг звонка (`call_offer`/`call_ice_batch`/`call_response`/`call_end`)
идёт через `startService(intent)` на каждый сигнал отдельно
([MessengerService.kt:874](../app/src/main/java/com/example/test/MessengerService.kt:874)),
а лог пишется в `onStartCommand`, который вызывается заново на уже
работающем сервисе — легитимно, не пересоздание процесса.

**Настоящая причина**: серверный лог показал, что токен `…9bc2b4` уже был
использован и подтверждён доставленным ~12 минут раньше (во время самого
звонка), а телефон использовал его **повторно** для нового, не связанного
со звонком сообщения. Сервер закономерно ответил "офлайн" — маршрут для
этого одноразового токена уже был стёрт после первой успешной доставки
(`token_to_ws.pop` в обработчике `anon_delivery_ack`).

Источник — механизм `AnonTokenManager.restoreContactToken()` (добавлен
в этой же сессии, "не терять токен при явном сбое отправки"): он
возвращает токен обратно в локальный пул, если `sendWs()` вернул `false`.
Под сетевой нагрузкой (видеозвонок — камера, кодирование, WebRTC — именно
такая нагрузка) возможна гонка: `sendWs()` локально считает отправку
неудавшейся, токен восстанавливается для повторного использования — но
пакет **всё равно** доходит до сервера и получает `anon_delivery_ack`.
Итог — один и тот же одноразовый токен потрачен дважды: один раз по
назначению, второй раз в пустоту, с сообщением-отправителем, уверенным
в успехе (`sendWs=true`), хотя доставка невозможна в принципе (сервер уже
стёр маршрут).

**Исправлено, по прямому запросу пользователя**: механизм
восстановления токена в пул **отключён** во всех трёх местах
(`sendWithForwardSecrecy`, `sendAnonOrDirect`, `sendAnonTokensTo`) —
вызовы `AnonTokenManager.restoreContactToken(...)` закомментированы, сама
функция в `AnonTokenManager.kt` оставлена нетронутой (не удалена, просто
не вызывается). Само сообщение по-прежнему ставится в очередь на повтор
(`queuePendingAnon`) — меняется только судьба токена: он теперь просто
теряется при неудачной отправке (как было до фикса из middle сессии),
а не рискует быть использован дважды. Осознанный компромисс: изредка
потерянный токен — цена, которую готовы платить, чтобы никогда не слать
сообщение молча в никуда с виду успешно.

**Не сделано**: точный механизм гонки (что именно в OkHttp/`sendWs()`
может сообщить `false`, когда кадр всё-таки ушёл) не подтверждён
инструментально — гипотеза основана на корреляции с моментом видеозвонка
и логике "иначе токен не мог быть использован дважды". Диагностика
consume/restore не добавлялась, поскольку сам механизм restore теперь
выключен — специально ловить нечего.

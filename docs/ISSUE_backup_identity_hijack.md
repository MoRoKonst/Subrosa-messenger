...
**A0 — не баги, уже реализовано, нужна только живая проверка (не путать
с "не начато"):**
- Судьба старых Double Ratchet-сессий устройства при восстановлении
  бэкапа на уже активном устройстве — SCENARIOS.md на момент триажа
  утверждал "не продумано", но `wipeCurrentIdentityData()`
  (`BackupManager.kt`) уже безусловно вызывает
  `SessionKeyManager.deleteAllSessions()` перед импортом на **любом**
  устройстве, не только "чистом". Похоже, уже закрыто фактически —
  SCENARIOS.md по этому пункту сам устарел, нужно перепроверить и
  поправить формулировку, а не писать новый код.
- "Бэкап не переносит состояние сессий → входящие не расшифруются" —
  `setPendingSessionResetContacts`/`getAndClearPendingSessionResetContacts`
  (`UserStorage.kt`) уже вызываются из `BackupManager.importBackup()` и
  `MessengerService.kt` — восстановленным контактам после импорта
  рассылается `session_reset`. Тоже похоже на уже закрытое, надо
  перепроверить и поправить SCENARIOS.md, не писать новый код.

**A1 — тривиально:**
- `group_reaction` не проверяет подпись — просто добавить тот же вызов
  `CryptoManager.verify`, что уже есть у `group_message`/`group_create`.
- `WipeManager.Level.SOFT` — мёртвый код, никуда не подключён. Если
  решаем не давать ему смысла — просто удалить enum-значение и
  `when`-ветку.

**A2 — низкая/средняя сложность, ясен путь решения:**
- `file_name` в `file_chunk` уходит открытым текстом — зашифровать это
  поле тем же ключом, что и `data`, либо заменить на generic-имя +
  расширение по MIME. Обе платформы, чанкующий код уже на месте.
- Голосовые сообщения при отсутствии ключа получателя ломаются через
  чужую (текстовую) очередь ретраев — завести отдельную
  `pendingVoiceSends`, по образцу уже существующих
  `pendingImages`/`pendingFileSends`.
- Повторный импорт одного бэкапа дублирует историю сообщений — включить
  в экспорт исходные ID сообщений, на импорте делать upsert/skip по ID
  вместо безусловного добавления.
- Ротация SPK / republish бандла держится только на частоте реконнектов
  — добавить независимый периодический таймер (коротко: ещё одна
  `while (isActive) { delay(...) }`-петля рядом с уже существующими
  30с/60с петлями в `MessengerService.kt`/`WebSocketClient.kt`).

**A3 — средняя сложность (трогает протокол, обе платформы):**
- Групповой чат: при добавлении участника остальные не узнают о нём —
  включить полный `members`/`admins` в сам пакет `group_create`
  (подписанный) + разослать уведомление о пополнении всем существующим
  участникам, симметрично уже работающему пути удаления. Нужно менять
  и Android, и Desktop, и формат пакета.
- Война переподключений для аккаунтов без TOTP — при получении
  `session_conflict` не переподключаться тем же безусловным циклом, что
  при обычном обрыве; требовать явного действия пользователя перед
  повторным `register`. Риск: не сломать нормальные реконнекты после
  реального обрыва связи — нужно аккуратно отличать эти два случая.

**A4 — средняя/высокая сложность (конкурентность на сервере):**
- OPK может быть выдан дважды при гонке двух одновременных запросов
  бандла — нужна атомарная операция "выдать и сразу вычеркнуть" в
  обработчике `get_prekey_bundle` (`server.py`), а не текущее
  "прочитать, потом отдельно удалить". Требует аккуратного тестирования
  под конкурентной нагрузкой, не только чтения кода.

### B. Не знаем, как чинить полностью — нужно исследование или решение по продукту

**B1 — нужно решение по продукту, не техническая сложность:**
- `WipeManager.Level.SOFT` — если всё-таки не удалять, а дать ему
  смысл: что именно должен стирать "мягкий" вайп, отличный от HARD?
  Нет черновика даже направления, нужен отдельный разговор.

**B2 — зависит от поведения ОС, решение может оказаться "жить с
ограничением", а не фикс:**
- Dead man's switch: не проверено и не ясно, сработает ли будильник
  корректно, если устройство было выключено дольше, чем
  интервал+grace period. Зависит от гарантий `AlarmManager`/Doze на
  конкретных прошивках (уже видели похожую историю с 16 KB
  page-size — "какой-то хернёй андроид заставляет заниматься"). Нужно
  либо живое исследование на разных устройствах, либо осознанно
  документировать как известное ограничение (сработает при следующем
  открытии приложения, не гарантированно в фоне) вместо попытки
  гарантировать то, что ОС не гарантирует.
...
**Главный вывод**: почти всё в категории A имеет понятный путь решения
и в основном локализовано — либо один файл/протокольное поле, либо
симметричный уже существующему коду (удаление участника, review чанков
и т.п.). Категория B — два пункта, оба не про "не умеем писать код", а
про "нужно решение/исследование, прежде чем писать код". Ничего в этом
списке не проверялось вживую на устройстве — это отдельная, сквозная
задача, не привязанная к конкретному пункту.
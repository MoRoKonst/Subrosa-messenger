...
## Тир 5 (отложено) — "Смена identity" не отзывает старый ключ на сервере

Найдено при написании гида по функциям безопасности (`SecurityGuideScreen.kt`) —
пользователь сам заметил дыру, спросив прямым текстом. `BackupManager.
...
отправляя на сервер. `server.py` вообще не имеет понятия "отозванный ключ" —
никакого списка/механизма ревокации не существует.

Реальное следствие: если сценарий — "ключ реально украли" (а не просто
паранойя), сброс identity защищает только *владельца* (у него новый ключ),
но никак не мешает *атакующему*, у которого есть копия старого приватного
ключа:
1. Атакующий всё ещё проходит challenge-response под старым fingerprint —
   сервер не отказывает, ключ технически валиден.
2. Атакующий может выдавать себя за жертву перед контактами, которые ещё
   не обменялись с ней свежим инвайт-кодом (у них в кэше старый публичный
   ключ жертвы — расшифровать/подписаться под ним может кто угодно с
   приватным ключом, то есть атакующий).
3. Атакующий может продолжать получать/отдавать prekey bundle под старой
   identity, заводя новые X3DH-сессии с не предупреждёнными контактами.

Предложенное решение (не реализовано, обсуждалось с пользователем
2026-08-13, отложено по его решению — "зафиксировать как задачу на потом"):
серверная ревокация. Пока старый приватный ключ ещё жив локально (в момент
нажатия кнопки, до самого вайпа), клиент подписывает им сообщение-отзыв
(`{"type": "revoke_identity", "fingerprint": ..., "signature": ...}`) и
отправляет на сервер ДО удаления. Сервер проверяет подпись против уже
известного публичного ключа для этого fingerprint, добавляет fingerprint в
персистентный список отозванных, и после этого:
- отказывает в challenge-response для этого публичного ключа;
- перестаёт отдавать prekey bundle для него (или отдаёт с явной пометкой
  "revoked", чтобы клиент контакта мог предупредить пользователя).

Затронет: Android (`MessengerService.kt`/`BackupManager.kt`), Desktop
(`WebSocketClient.kt`/аналог `BackupManager`), и `server.py` на ВПС (новый
тип сообщения + персистентное хранилище отозванных ключей — сейчас все
структуры типа `known_tokens`/`token_to_ws` только in-memory, нужно решить,
переживает ли список отзыва рестарт сервера, вероятно да).
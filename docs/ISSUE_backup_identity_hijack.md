...
15. **TOTP 2FA на бэкап** — отдельная большая фича, отвечает на исходный
    вопрос всего файла (второй фактор для восстановления identity).
    Нужен отдельный дизайн-проход (хранение секрета, UX ввода кода,
    восстановление при потере TOTP-устройства).

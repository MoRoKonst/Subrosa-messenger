...
**Побочно найдено, не исправлено**: `processSessionInit()` (принимающая
сторона первого контакта — сюда попадает получатель session_init) кэширует и
персистит классический ключ отправителя (`publicKeys[from]` +
`ChatStorage.saveContactPublicKey`), но **не** PQ-ключ — тот попадает в
кэш только когда это устройство само явно запросит prekey bundle через
`requestPrekeyBundle()` (лениво, по требованию, при первой попытке
гибридной отправки не-through-session путём). То есть принимающая
сторона первого контакта в принципе никогда не получает PQ-ключ
собеседника даром вместе с session_init — это отдельный, не
исправленный сейчас разрыв, не по вине рестартов. Desktop
(`WebSocketClient.kt`, не в гите) имеет структурно тот же паттерн
(`peerPqPublicKeys`, in-memory) — не проверялся и не трогался в этом
проходе.
...
убедиться, что PQ-ключ не запрашивается заново).

...
- Война переподключений для аккаунтов без TOTP — **это единственный
  пункт из всего первоначального списка A2/A3, который подтверждён всё
  ещё открытым**: `session_conflict`-обработчик (`MessengerService.kt:1300`)
  просто закрывает сокет и показывает уведомление, ничего не мешает
  внешнему циклу `connect()` безусловно переподключиться. Путь решения
  прежний: не переподключаться автоматически сразу после
  `session_conflict`, требовать явного действия пользователя.

...
*Last verified against commit: `2ae787a` (2026-08-28) — re-verified end-to-end against an external documentation-level review of this exact file (see `docs/KNOWN_SECURITY_ISSUES.md` § S5 for what changed and why). This project changes quickly — if you're reading this much later than that date, treat specific claims as needing a fresh code check, not as guaranteed-current.*
...
18. ~~Call signaling was never anonymized, to avoid a reliability bug~~ — **fixed via a two-phase request/response flow** (1:1 calls only; `call_group_*` group-call signaling is unaffected by this item and still always goes out directly, see below). The original problem: routing `call_offer`/`call_ice`/etc. through `anon_message` meant a recipient who was even momentarily offline (a brief reconnect, a network blip) got the message silently queued instead of delivered live — no live-delivery guarantee, and no missed-call/FCM-wake fallback for anon-routed traffic (that only exists on the server's *direct* call-relay path). A late-delivered `call_offer` is useless once the caller has already timed out, and a queued `call_end` could leave the other side's call UI hanging. This was reproduced live via matched client/server logs (`[ANON] Токен ... офлайн, сообщение в очереди` at the exact moment a call attempt silently failed) and initially "fixed" by routing all call signaling direct — closing the reliability bug but reopening the metadata leak (server sees who calls whom and when).
    ...
    **This conclusion is stale, found during a website copy audit 2026-08-19**: `call_offer`/`answer`/`ice`/`end` for 1:1 calls route through `sendAnonOrDirect()` — the same shared path used by reactions, edits, typing, read receipts, and voice notes. That function's direct fallback was removed entirely by a separate, later decision this same session (see "Decided: remove `sendAnonOrDirect`'s direct fallback entirely" in `docs/ISSUE_backup_identity_hijack.md`) — on a token shortage the message now queues (`queuePendingAnon`) instead of going out fingerprint-addressed. The SDP/ICE reliability problem that motivated reverting to direct addressing above has since been fixed by a different, more general fix (the session-sync race, the queue TTL, the real delivery-ack model — see `ISSUE_backup_identity_hijack.md`'s 2026-08-17/18 findings), so real SDP/ICE signaling for 1:1 calls is **now fully anonymized**, with no direct fallback at all. Group calls (`call_group_*`) are unaffected — they still go out directly via `sendWs()`, deliberately and permanently, outside the shared `sendAnonOrDirect` path.
...
27. **No protection against offline access to the server's message database — mitigated, opt-in**: `server.py`'s SQLite database (`messages.db`) was always stored unencrypted on disk — a stolen disk, an unencrypted backup, or a hosting provider with filesystem access could read it directly, including the permanent `registered_fingerprints`/`revoked_fingerprints` tables and (unless `TOTP_SECRET_ENCRYPTION_KEY` is also set) plaintext TOTP secrets. Added optional SQLCipher-backed encryption at rest, enabled by setting `DB_ENCRYPTION_KEY_HEX` (a 32-byte hex key) in the server's environment; off by default, so existing deployments are unaffected until an operator opts in. **This defends only against offline/at-rest access** — a root-level attacker on the *live* process can still read the key from the environment, same limitation as `TOTP_SECRET_ENCRYPTION_KEY`. Relayed message payloads themselves are opaque ciphertext either way (server never has the E2EE keys), so this closes a metadata gap, not a content one. Enabling it on an existing deployment requires starting from an empty database — SQLCipher cannot open a file that was written in plain SQLite format.

28. **Calculator disguise had a hardcoded, publicly-known unlock code — fixed, and its interaction with panic-wipe/decoy-restore had a data-loss bug — also fixed**: the icon/launcher-swap disguise (`CalculatorScreen.kt`) previously unlocked on an exact fixed keystroke sequence (`4 + 20 =`), which is fine for a one-off private build but not a real secret once the source is public — so the setting was hidden from the UI entirely. It's now exposed with a user-chosen unlock code: any expression whose result matches a configured number unlocks the app, with no hardcoded fallback value at all (if no code is set, the disguise cannot be unlocked via the calculator). Enabling it requires explicitly saving a valid code first — toggling the setting alone no longer swaps the launcher component (which kills the app process) before the user has had a chance to set their code. Separately, and more seriously: the panic-wipe and decoy-restore paths (`WipeManager.hardWipe()`/`wipeForDecoyKeepAlive()`, `MessengerService.performEmergencyWipe()`, restored via `UserStorage.migrateDecoyState()`) carry the "disguise enabled" flag across a wipe so a decoy profile keeps its calculator front end, but were not carrying the unlock code alongside it — the code lived in the same encrypted-prefs file being wiped. A wipe-then-restore cycle would have re-enabled the disguise with no way to unlock it, permanently locking the user out of their own restored profile. Fixed by capturing and restoring the code together with the flag at all three wipe sites, and by only re-enabling the disguise on restore if its code survived intact.

29. **Only one invite-code mailbox tag can be "live" per device at a time**: each invite code embeds a one-time anonymous-mailbox tag (see [Invite Codes](#invite-codes)), and generating a fresh code (tapping Copy/Share/Show-QR in Profile) replaces the single tag the device is currently polling for. If a code has already been given to someone who hasn't redeemed it yet, and a second code is then generated for a different person, the first code still looks structurally valid (signature and TTL both check out) but its mailbox deposit is never picked up — the first person's connection attempt fails silently, with no error on either side. This is a deliberate tradeoff, not an oversight: minting additional tags increases the number of distinct tags the anonymous-mailbox/cover-traffic system has to carry, which is a cost for every user's traffic-analysis resistance, not just the person generating them — supporting multiple simultaneously-live tags per device was considered and rejected on that basis. The UI (Profile screen and the in-app security guide) now warns explicitly not to regenerate a code until its recipient has connected.

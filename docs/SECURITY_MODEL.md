...
| Protection against a fully compromised, unlocked endpoint | If the OS/device is rooted and an attacker has live RAM access while the app is unlocked, the Storage Master Key is in memory — unavoidable for any app that needs to actually decrypt content for the user. | [SECURITY.md § Threat Model, Out of scope](SECURITY.md#threat-model) |
